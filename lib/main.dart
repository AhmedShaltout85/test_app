...
class _HydraulicPumpChecklistState extends State<HydraulicPumpChecklist> {
  ...
  Future<void> _loadArabicFont() async {
    try {
      ...
      _arabicFont = await _getFallbackFont();
      setState(() {
        _fontLoaded = true;
      });
    }
  ...
  Future<pw.Font> _getFallbackFont() async {
    // Try to use a basic font that might support Arabic
    try {
      final fontData = await rootBundle.load('fonts/arial.ttf');
      return pw.Font.ttf(fontData);
    } catch (e) {
      // Final fallback - use the default font
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: _isGeneratingPdf || !_fontLoaded
                ? const SizedBox(
                    width: 20,
                    height: 20,
            ...
            onPressed:
                (_isGeneratingPdf || !_fontLoaded) ? null : _saveFormData,
      ...
      body: (!_fontLoaded || _isGeneratingPdf)
          ? const Center(
              child: Column(
...
//                   )
//                 : const Icon(Icons.save),
//             onPressed: _isGeneratingPdf ? null : _saveFormData,
//             tooltip: 'Save Form and Generate PDF',
//           ),
//           IconButton(
//             icon: const Icon(Icons.clear),
